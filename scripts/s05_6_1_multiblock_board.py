...
PORT = os.environ.get("S05_SERIAL", os.environ.get("S05_6_1_SERIAL", "/dev/ttyUSB1"))
BAUD = int(os.environ.get("S05_BAUD", "115200"))
SOURCE_PATH = Path("runtime_c/gemv_multiblock_test.c")
LOG_PATH = Path(os.environ.get("S05_6_1_BOARD_LOG", "logs/s05_6_1_multiblock_board.txt"))
CSV_PATH = Path(os.environ.get("S05_6_1_BOARD_CSV", "reports/s05_6_1_multiblock_board.csv"))
BOARD_SOURCE = "/tmp/s05_6_1_gemv_multiblock_test.c"
BOARD_BIN = "/tmp/s05_6_1_gemv_multiblock_test"
PHYS_BASE = os.environ.get("S05_PHYS_BASE", "0x3c000000")
PHYS_SIZE = os.environ.get("S05_PHYS_SIZE", "0x04000000")
RUN_TIMEOUT = int(os.environ.get("S05_6_1_RUN_TIMEOUT", "240"))
EXTRA_ARGS = os.environ.get("S05_6_1_EXTRA_ARGS", "").strip()
...
class Session:
    ...
    def cmd(self, command, timeout=30):
        ...
        self.buf.clear()
        self.cmd_seq += 1
        done = f"__S05_6_1_CMD_DONE_{self.cmd_seq}__"
...
def extract_csv():
    lines = [
        "case,shape,pattern,blocks_per_row,packet_bytes,result,classification,first_mismatch,got,expected,elapsed_us,mm2s_sr,s2mm_sr,status,debug_row,debug_block,debug_lane,debug_in_count,debug_tlast_count"
    ]
    for line in LOG_PATH.read_text(encoding="ascii", errors="replace").splitlines():
        if line.startswith("BOARDCSV,"):
...
def main():
    ...
    try:
        ...
        if not s.cmd(f"rm -f {BOARD_SOURCE} {BOARD_BIN}", timeout=10):
            return 4
        upload = (
            f"cat > {BOARD_SOURCE} <<'__S05_6_1_C_SOURCE__'\n"
            f"{source}"
            "__S05_6_1_C_SOURCE__\n"
            f"wc -c {BOARD_SOURCE}"
        )
        ...
        compile_cmd = (
            ...
            "rc=$?; echo __S05_6_1_COMPILE_RC__=$rc; "
            f"[ $rc -eq 0 ] && ls -l {BOARD_BIN}"
        )
        if not s.cmd(compile_cmd, timeout=120):
            return 5
        if b"__S05_6_1_COMPILE_RC__=0" not in LOG_PATH.read_bytes():
            print("FAIL: board compile failed", file=sys.stderr)
            return 5

        run_cmd = (
            "echo __S05_6_1_RUN_START__;"
            f"{BOARD_BIN} --phys-base {PHYS_BASE} --phys-size {PHYS_SIZE} {EXTRA_ARGS}; "
            "rc=$?; echo __S05_6_1_RUN_RC__=$rc; "
            "echo __S05_6_1_DMESG_CHECK__;"
            "dmesg | tail -120 | grep -Ei 'panic|oops|bug:|bus error|external abort|imprecise|segfault|DMA|uio|gemv|xilinx' || true;"
            "echo __S05_6_1_RUN_END__"
        )
        if not s.cmd(run_cmd, timeout=RUN_TIMEOUT):
            return 6
        extract_csv()
        data = LOG_PATH.read_bytes()
        if b"S05_6_1_BOARD_PASS" not in data:
