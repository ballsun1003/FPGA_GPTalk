...
module gemv_q8_0_stream_core #(
    ...
    parameter integer DEBUG_PROBE_WORDS = 16
) (
    input wire clk,
    input wire reset_p,
    ...
    input wire debug_probe_enable,
    input wire [1:0] debug_probe_select,
    ...
    wire [FEATURE_WIDTH-1:0] debug_target_block =
        {{(FEATURE_WIDTH-1){1'b0}}, debug_probe_select[1]};
    wire [15:0] debug_target_lane = debug_probe_select[0] ? 16'd12 : 16'd4;
    wire debug_target_block_match = debug_probe_enable && (block_index == debug_target_block);
    wire debug_target_emit_match = debug_target_block_match && (emit_lane == debug_target_lane);
    wire debug_target_apply_match = debug_target_block_match && (debug_target_lane < LANES_U16);
    wire [31:0] debug_input_index = {{(32-INPUT_ADDR_WIDTH){1'b0}}, input_rd_addr};
        ...
        if (reset_p) begin
            ...
            case (state)
                ...
                ST_WEIGHT_APPLY: begin
                    ...
                    debug_lane <= apply_lane_base;
	                    if (TDATA_WIDTH == 128) begin
                            ...
                            if (debug_target_apply_match) begin
                                debug_probe[(DBG_W_INPUT_INDEX*32) +: 32] <= debug_input_index;
                                debug_probe[(DBG_W_INPUT_I16*32) +: 32] <= sext_i16_to_i32(input_sample_reg);
                                debug_probe[(DBG_W_WEIGHT_I8*32) +: 32] <= sext_i8_to_i32(get_weight_byte(weight_word_ext, debug_target_lane));
                                debug_probe[(DBG_W_PRODUCT*32) +: 32] <=
                                    mul_i16_i8_to_i32(input_sample_reg, get_weight_byte(weight_word_ext, debug_target_lane));
                                debug_probe[(DBG_W_BLOCK_BEFORE*32) +: 32] <= block_acc[debug_target_lane];
                                debug_probe[(DBG_W_BLOCK_AFTER*32) +: 32] <=
                                    block_acc[debug_target_lane] +
                                    mul_i16_i8_to_i32(input_sample_reg, get_weight_byte(weight_word_ext, debug_target_lane));
                                debug_probe[(DBG_W_BLOCK_LANE*32) +: 32] <= {block_index[15:0], debug_target_lane};
                                debug_probe[(DBG_W_BEAT_IDX*32) +: 32] <= weight_beat_idx_reg;
                            end
	                    end else begin
                ...
                ST_SCALE_ACCUM: begin
	                    ...
	                    debug_block <= block_index;
	                    debug_lane <= emit_lane;
	                    if (debug_target_emit_match) begin
	                        debug_probe[(DBG_W_SCALE_Q*32) +: 32] <= scale_q_reg;
	                        debug_probe[(DBG_W_SCALED*32) +: 32] <= scaled_shifted_scalar[31:0];
	                        debug_probe[(DBG_W_ROW_BEFORE*32) +: 32] <= row_acc[emit_lane][31:0];
	                        debug_probe[(DBG_W_ROW_AFTER*32) +: 32] <= row_acc_after_scalar[31:0];
	                    end
                ...
                ST_EMIT_ROW: begin
                    if (m_axis_tvalid) begin
                        if (m_axis_tready) begin
                            ...
                            if (debug_probe_enable && (block_index == debug_target_block) && (emit_lane == debug_target_lane)) begin
                                debug_probe[(DBG_W_OUTPUT_WORD*32) +: 32] <= row_out[emit_lane];
                            end
	                        m_axis_tvalid <= 1'b1;
                ...
                ST_EMIT_BLOCK: begin
                    if (m_axis_tvalid) begin
                        if (m_axis_tready) begin
                            ...
                            if (debug_target_emit_match) begin
                                debug_probe[(DBG_W_OUTPUT_WORD*32) +: 32] <= block_acc[emit_lane];
                            end
	                        m_axis_tvalid <= 1'b1;
