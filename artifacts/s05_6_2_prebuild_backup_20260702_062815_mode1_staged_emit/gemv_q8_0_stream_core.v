...
module gemv_q8_0_stream_core #(
    ...
    localparam [4:0] ST_BLOCK_STAGE_LOAD  = 5'd17;
    localparam [4:0] ST_BLOCK_STAGE_STORE = 5'd18;
    ...
    always @(posedge clk) begin
        if (reset_p) begin
            ...
            case (state)
                ...
                ST_BLOCK_STAGE_LOAD: begin
                    debug_row <= row_group_base;
                    debug_block <= block_index;
                    debug_lane <= emit_lane;
                    scale_block_acc_reg <= block_acc[emit_lane];
                    state <= ST_BLOCK_STAGE_STORE;
                end

                ST_BLOCK_STAGE_STORE: begin
                    debug_row <= row_group_base;
                    debug_block <= block_index;
                    debug_lane <= emit_lane;
                    row_out[emit_lane] <= scale_block_acc_reg;
