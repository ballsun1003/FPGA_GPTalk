...
enum {
    ...
    GEMV_DEBUG_PROBE_SELECT = 0xa8,
    GEMV_DEBUG_PROBE_INDEX = 0xac,
    GEMV_DEBUG_PROBE_DATA = 0xb0,
...
static uint32_t read_probe_word(volatile uint32_t *gemv, uint32_t index)
{
    wr32(gemv, GEMV_DEBUG_PROBE_INDEX, index);
    return rd32(gemv, GEMV_DEBUG_PROBE_DATA);
}

static void configure_probe(volatile uint32_t *gemv, int debug_select)
{
    if (debug_select >= 0) {
        wr32(gemv, GEMV_DEBUG_PROBE_SELECT, 1u | (((uint32_t)debug_select & 3u) << 1));
    } else {
        wr32(gemv, GEMV_DEBUG_PROBE_SELECT, 0u);
    }
}

static const char *probe_name(int debug_select)
{
    static const char *names[4] = {"b0_l4", "b0_l12", "b1_l4", "b1_l12"};
    if (debug_select < 0 || debug_select > 3) return "disabled";
    return names[debug_select];
}

static void dump_narrow_debug(struct hw *hw, const char *case_name, const char *mode_name,
                              int debug_select)
{
    enum {
        PROBE_INPUT_INDEX = 0,
        ...
        PROBE_WEIGHT_I8 = 2,
        PROBE_PRODUCT = 3,
        PROBE_BLOCK_BEFORE = 4,
        PROBE_BLOCK_AFTER = 5,
        PROBE_SCALE_Q = 6,
        PROBE_SCALED = 7,
        PROBE_ROW_BEFORE = 8,
        PROBE_ROW_AFTER = 9,
        PROBE_OUTPUT_WORD = 10,
        PROBE_S2MM_TDATA = 11,
        PROBE_S2MM_KEEP_LAST = 12,
        PROBE_BLOCK_LANE = 13,
        PROBE_BEAT_IDX = 14,
        PROBE_OUTPUT_BEAT_IDX = 15
    };

    if (debug_select < 0) return;
    uint32_t keep_last = read_probe_word(hw->gemv, PROBE_S2MM_KEEP_LAST);
    uint32_t block_lane = read_probe_word(hw->gemv, PROBE_BLOCK_LANE);
    printf("NARROWDBG,%s,%s,%s"
           ",input_index=%u,input_i16=%" PRId32 ",weight_i8=%" PRId32
           ",product=%" PRId32 ",block_before=%" PRId32 ",block_after=%" PRId32
           ",scale_q=%" PRId32 ",scaled=%" PRId32
           ",row_before=%" PRId32 ",row_after=%" PRId32
           ",output_word=%" PRId32
           ",s2mm_tdata=%" PRId32 ",s2mm_tkeep=0x%x,s2mm_tlast=%u"
           ",block_idx=%u,lane_idx=%u,beat_idx=%u,output_beat_idx=%u\n",
           case_name, mode_name, probe_name(debug_select),
           read_probe_word(hw->gemv, PROBE_INPUT_INDEX),
           (int32_t)read_probe_word(hw->gemv, PROBE_INPUT_I16),
           (int32_t)read_probe_word(hw->gemv, PROBE_WEIGHT_I8),
           (int32_t)read_probe_word(hw->gemv, PROBE_PRODUCT),
           (int32_t)read_probe_word(hw->gemv, PROBE_BLOCK_BEFORE),
           (int32_t)read_probe_word(hw->gemv, PROBE_BLOCK_AFTER),
           (int32_t)read_probe_word(hw->gemv, PROBE_SCALE_Q),
           (int32_t)read_probe_word(hw->gemv, PROBE_SCALED),
           (int32_t)read_probe_word(hw->gemv, PROBE_ROW_BEFORE),
           (int32_t)read_probe_word(hw->gemv, PROBE_ROW_AFTER),
           (int32_t)read_probe_word(hw->gemv, PROBE_OUTPUT_WORD),
           (int32_t)read_probe_word(hw->gemv, PROBE_S2MM_TDATA),
           keep_last & 0xfu,
           (keep_last >> 4) & 0x1u,
           (block_lane >> 16) & 0xffffu,
           block_lane & 0xffffu,
           read_probe_word(hw->gemv, PROBE_BEAT_IDX),
           read_probe_word(hw->gemv, PROBE_OUTPUT_BEAT_IDX));
}
...
static int run_case(struct hw *hw, const struct test_case *tc, int debug_select, int dump_debug_pass)
{
    ...
    configure_probe(hw->gemv, debug_select);
    ...
    if (debug_select >= 0 && (failed || dump_debug_pass)) {
        dump_narrow_debug(hw, tc->name, "mode0", debug_select);
    }
    return failed;
}

static int run_case_mode1_blocks(struct hw *hw, const struct test_case *tc,
                                 int debug_select, int dump_debug_pass)
{
    static int16_t input[MAX_IN_FEATURES];
    static uint8_t packet[MAX_PACKET_BYTES];
    ...
    if (debug_select >= 0 && (failed || dump_debug_pass)) {
        dump_narrow_debug(hw, tc->name, "mode1", debug_select);
    }
    return failed;
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "--phys-base") == 0 && i + 1 < argc) {
            ...
            mode1_blocks = 1;
        } else if (strcmp(argv[i], "--debug-select") == 0 && i + 1 < argc) {
            debug_select = atoi(argv[++i]);
            if (debug_select < 0 || debug_select > 3) {
                fprintf(stderr, "--debug-select must be 0..3\n");
                return 2;
            }
        } else if (strcmp(argv[i], "--dump-debug-pass") == 0) {
            dump_debug_pass = 1;
        } else {
            fprintf(stderr, "usage: %s [--phys-base ADDR] [--phys-size BYTES] [--only CASE] [--repeat N] [--mode1-blocks] [--debug-select 0..3] [--dump-debug-pass]\n", argv[0]);
            return 2;
        }
    }
    ...
    for (int r = 0; r < repeat; ++r) {
        for (size_t i = 0; i < sizeof(tests) / sizeof(tests[0]); ++i) {
            ...
            selected_count++;
            fail_count += mode1_blocks ?
                run_case_mode1_blocks(&hw, &tests[i], debug_select, dump_debug_pass) :
                run_case(&hw, &tests[i], debug_select, dump_debug_pass);
        }
    }
