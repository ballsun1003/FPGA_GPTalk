...
- Current source is the `fixed_128_mac` functional base plus the `mode1_final_apply_snapshot` delta.
...
- The `mode1_final_apply_snapshot` delta changes mode1 only: during `ST_WEIGHT_APPLY`, the same next-value written to `block_acc[]` is also written to existing `row_out[]` when `mode_reg=1`; `ST_EMIT_BLOCK` then emits `row_out[]`. This avoids post-block dynamic reads of `block_acc[]`.
