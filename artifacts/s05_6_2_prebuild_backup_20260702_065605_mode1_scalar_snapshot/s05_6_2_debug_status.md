...
- Current source is the `fixed_128_mac` functional base plus the `mode1_scalar_snapshot` delta.
...
- The `mode1_scalar_snapshot` delta changes mode1 only: at `ST_BLOCK_DONE`, it copies `block_acc[0..15]` into 16 scalar `mode1_out*` registers when `mode_reg=1`; `ST_EMIT_BLOCK` emits those scalar registers through a case-style mux. This avoids dynamic array reads from `block_acc[]` during mode1 output emission and avoids sharing mode0 `row_out[]`.
