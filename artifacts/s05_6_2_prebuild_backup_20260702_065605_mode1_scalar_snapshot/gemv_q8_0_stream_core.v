...
module gemv_q8_0_stream_core #(
    ...
    reg signed [31:0] mode1_out0;
    reg signed [31:0] mode1_out1;
    reg signed [31:0] mode1_out2;
    reg signed [31:0] mode1_out3;
    reg signed [31:0] mode1_out4;
    reg signed [31:0] mode1_out5;
    reg signed [31:0] mode1_out6;
    reg signed [31:0] mode1_out7;
    reg signed [31:0] mode1_out8;
    reg signed [31:0] mode1_out9;
    reg signed [31:0] mode1_out10;
    reg signed [31:0] mode1_out11;
    reg signed [31:0] mode1_out12;
    reg signed [31:0] mode1_out13;
    reg signed [31:0] mode1_out14;
    reg signed [31:0] mode1_out15;
    ...
    wire signed [31:0] mode1_emit_data =
        (emit_lane == 16'd0)  ? mode1_out0  :
        (emit_lane == 16'd1)  ? mode1_out1  :
        (emit_lane == 16'd2)  ? mode1_out2  :
        (emit_lane == 16'd3)  ? mode1_out3  :
        (emit_lane == 16'd4)  ? mode1_out4  :
        (emit_lane == 16'd5)  ? mode1_out5  :
        (emit_lane == 16'd6)  ? mode1_out6  :
        (emit_lane == 16'd7)  ? mode1_out7  :
        (emit_lane == 16'd8)  ? mode1_out8  :
        (emit_lane == 16'd9)  ? mode1_out9  :
        (emit_lane == 16'd10) ? mode1_out10 :
        (emit_lane == 16'd11) ? mode1_out11 :
        (emit_lane == 16'd12) ? mode1_out12 :
        (emit_lane == 16'd13) ? mode1_out13 :
        (emit_lane == 16'd14) ? mode1_out14 :
        (emit_lane == 16'd15) ? mode1_out15 : 32'sd0;
        ...
        if (reset_p) begin
            ...
            case (state)
                ...
                ST_BLOCK_DONE: begin
                    ...
                    debug_lane <= 16'd0;
                    if (mode_reg) begin
                        mode1_out0 <= block_acc[0];
                        mode1_out1 <= block_acc[1];
                        mode1_out2 <= block_acc[2];
                        mode1_out3 <= block_acc[3];
                        mode1_out4 <= block_acc[4];
                        mode1_out5 <= block_acc[5];
                        mode1_out6 <= block_acc[6];
                        mode1_out7 <= block_acc[7];
                        mode1_out8 <= block_acc[8];
                        mode1_out9 <= block_acc[9];
                        mode1_out10 <= block_acc[10];
                        mode1_out11 <= block_acc[11];
                        mode1_out12 <= block_acc[12];
                        mode1_out13 <= block_acc[13];
                        mode1_out14 <= block_acc[14];
                        mode1_out15 <= block_acc[15];
                        emit_lane <= 16'd0;
                        m_axis_tvalid <= 1'b0;
                        state <= ST_EMIT_BLOCK;
                    end else begin
                ...
                ST_EMIT_BLOCK: begin
                    if (m_axis_tvalid) begin
                        ...
                        m_axis_lane <= emit_lane;
                        if (row_group_base + emit_lane_feature == 0) begin
                            debug_out0 <= mode1_emit_data;
                        end else if (row_group_base + emit_lane_feature == 1) begin
                            debug_out1 <= mode1_emit_data;
                        end else if (row_group_base + emit_lane_feature == 2) begin
                            debug_out2 <= mode1_emit_data;
                        end
                        debug_row <= row_group_base + emit_lane_feature;
                        debug_block <= block_index;
                        debug_lane <= emit_lane;
