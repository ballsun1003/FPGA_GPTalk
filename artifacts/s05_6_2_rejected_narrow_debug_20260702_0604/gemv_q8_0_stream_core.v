...
module gemv_q8_0_stream_core #(
    ...
    parameter integer DEBUG_PROBE_WORDS = 64
) (
    input wire clk,
    input wire reset_p,
    ...
    output reg [(DEBUG_PROBE_WORDS*32)-1:0] debug_probe
);
    ...
    localparam integer DEBUG_RECORD_WORDS = 16;
    localparam integer DBG_REC_L4_B0 = 0;
    localparam integer DBG_REC_L12_B0 = 1;
    localparam integer DBG_REC_L4_B1 = 2;
    localparam integer DBG_REC_L12_B1 = 3;
    localparam integer DBG_W_INPUT_INDEX = 0;
    localparam integer DBG_W_INPUT_I16 = 1;
    localparam integer DBG_W_WEIGHT_I8 = 2;
    localparam integer DBG_W_PRODUCT = 3;
    localparam integer DBG_W_BLOCK_BEFORE = 4;
    localparam integer DBG_W_BLOCK_AFTER = 5;
    localparam integer DBG_W_SCALE_Q = 6;
    localparam integer DBG_W_SCALED = 7;
    localparam integer DBG_W_ROW_BEFORE = 8;
    localparam integer DBG_W_ROW_AFTER = 9;
    localparam integer DBG_W_OUTPUT_WORD = 10;
    localparam integer DBG_W_BLOCK_LANE = 13;
    localparam integer DBG_W_BEAT_IDX = 14;
    ...
    wire [31:0] debug_input_index =
        {{(32-INPUT_ADDR_WIDTH){1'b0}}, input_rd_addr};
    ...
    function signed [31:0] sext_i8_to_i32;
        input signed [7:0] value;
        begin
            sext_i8_to_i32 = {{24{value[7]}}, value};
        end
    endfunction

    function signed [31:0] sext_i16_to_i32;
        input signed [15:0] value;
        begin
            sext_i16_to_i32 = {{16{value[15]}}, value};
        end
    endfunction

    task latch_mac_probe;
        input integer rec;
        input [15:0] lane;
        input signed [7:0] weight_value;
        input signed [31:0] block_before;
        reg signed [31:0] product_value;
        reg signed [31:0] block_after;
        begin
            product_value = mul_i16_i8_to_i32(input_sample_reg, weight_value);
            block_after = block_before + product_value;
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_INPUT_INDEX)*32) +: 32] <= debug_input_index;
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_INPUT_I16)*32) +: 32] <= sext_i16_to_i32(input_sample_reg);
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_WEIGHT_I8)*32) +: 32] <= sext_i8_to_i32(weight_value);
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_PRODUCT)*32) +: 32] <= product_value;
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_BLOCK_BEFORE)*32) +: 32] <= block_before;
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_BLOCK_AFTER)*32) +: 32] <= block_after;
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_BLOCK_LANE)*32) +: 32] <= {block_index[15:0], lane};
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_BEAT_IDX)*32) +: 32] <= weight_beat_idx_reg;
        end
    endtask

    task latch_scale_probe;
        input integer rec;
        input signed [31:0] scale_value;
        input signed [63:0] scaled_value;
        input signed [ROW_ACC_WIDTH-1:0] row_before;
        input signed [ROW_ACC_WIDTH-1:0] row_after;
        begin
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_SCALE_Q)*32) +: 32] <= scale_value;
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_SCALED)*32) +: 32] <= scaled_value[31:0];
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_ROW_BEFORE)*32) +: 32] <= row_before[31:0];
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_ROW_AFTER)*32) +: 32] <= row_after[31:0];
        end
    endtask

    task latch_output_probe;
        input integer rec;
        input signed [31:0] output_value;
        begin
            debug_probe[((rec*DEBUG_RECORD_WORDS + DBG_W_OUTPUT_WORD)*32) +: 32] <= output_value;
        end
    endtask
    ...
    always @(posedge clk) begin
        if (reset_p) begin
            ...
            debug_probe <= {(DEBUG_PROBE_WORDS*32){1'b0}};
            ...
            case (state)
                ...
                ST_WEIGHT_RECV: begin
                    ...
                    if (s_axis_fire) begin
                        ...
                        if (!s_axis_tkeep_ok) begin
                            ...
                            weight_beat_idx_reg <= debug_in_count;
                            state <= ST_INPUT_WAIT;
                        end
                    end
                end
                ...
                ST_WEIGHT_APPLY: begin
                    ...
                    debug_lane <= apply_lane_base;
	                    if (TDATA_WIDTH == 128) begin
                            ...
                            if (block_index == {FEATURE_WIDTH{1'b0}}) begin
                                latch_mac_probe(DBG_REC_L4_B0, 16'd4, get_weight_byte(weight_word_ext, 4), block_acc[4]);
                                latch_mac_probe(DBG_REC_L12_B0, 16'd12, get_weight_byte(weight_word_ext, 12), block_acc[12]);
                            end else if (block_index == FEATURE_ONE) begin
                                latch_mac_probe(DBG_REC_L4_B1, 16'd4, get_weight_byte(weight_word_ext, 4), block_acc[4]);
                                latch_mac_probe(DBG_REC_L12_B1, 16'd12, get_weight_byte(weight_word_ext, 12), block_acc[12]);
                            end
	                    end else begin
                ...
                ST_SCALE_SHIFT: begin
	                ...
	                ST_SCALE_ACCUM: begin
	                    ...
	                    debug_lane <= emit_lane;
	                    if (block_index == {FEATURE_WIDTH{1'b0}}) begin
	                        if (emit_lane == 16'd4) begin
	                            latch_scale_probe(DBG_REC_L4_B0, scale_q_reg, scaled_shifted_scalar, row_acc[emit_lane], row_acc_after_scalar);
	                        end else if (emit_lane == 16'd12) begin
	                            latch_scale_probe(DBG_REC_L12_B0, scale_q_reg, scaled_shifted_scalar, row_acc[emit_lane], row_acc_after_scalar);
	                        end
	                    end else if (block_index == FEATURE_ONE) begin
	                        if (emit_lane == 16'd4) begin
	                            latch_scale_probe(DBG_REC_L4_B1, scale_q_reg, scaled_shifted_scalar, row_acc[emit_lane], row_acc_after_scalar);
	                        end else if (emit_lane == 16'd12) begin
	                            latch_scale_probe(DBG_REC_L12_B1, scale_q_reg, scaled_shifted_scalar, row_acc[emit_lane], row_acc_after_scalar);
	                        end
	                    end
                ...
                ST_EMIT_ROW: begin
                    if (m_axis_tvalid) begin
                        if (m_axis_tready) begin
                            ...
                            if (block_index == FEATURE_ONE) begin
                                if (emit_lane == 16'd4) begin
                                    latch_output_probe(DBG_REC_L4_B1, row_out[emit_lane]);
                                end else if (emit_lane == 16'd12) begin
                                    latch_output_probe(DBG_REC_L12_B1, row_out[emit_lane]);
                                end
                            end
	                        m_axis_tvalid <= 1'b1;
                ...
                ST_EMIT_BLOCK: begin
                    if (m_axis_tvalid) begin
                        if (m_axis_tready) begin
                            ...
                            if (block_index == {FEATURE_WIDTH{1'b0}}) begin
                                if (emit_lane == 16'd4) begin
                                    latch_output_probe(DBG_REC_L4_B0, block_acc[emit_lane]);
                                end else if (emit_lane == 16'd12) begin
                                    latch_output_probe(DBG_REC_L12_B0, block_acc[emit_lane]);
                                end
                            end else if (block_index == FEATURE_ONE) begin
                                if (emit_lane == 16'd4) begin
                                    latch_output_probe(DBG_REC_L4_B1, block_acc[emit_lane]);
                                end else if (emit_lane == 16'd12) begin
                                    latch_output_probe(DBG_REC_L12_B1, block_acc[emit_lane]);
                                end
                            end
	                        m_axis_tvalid <= 1'b1;
