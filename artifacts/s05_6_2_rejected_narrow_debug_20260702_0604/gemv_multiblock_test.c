...
enum {
    ...
    GEMV_DEBUG_PROBE_INDEX = 0xa8,
    GEMV_DEBUG_PROBE_DATA = 0xac,
...
{
    static const char *rec_names[4] = {"b0_l4", "b0_l12", "b1_l4", "b1_l12"};
    enum {
        PROBE_RECORD_WORDS = 16,
        PROBE_INPUT_INDEX = 0,
        PROBE_INPUT_I16 = 1,
        PROBE_WEIGHT_I8 = 2,
        PROBE_PRODUCT = 3,
    ...
    for (uint32_t rec = 0; rec < 4u; ++rec) {
        uint32_t base = rec * PROBE_RECORD_WORDS;
        uint32_t keep_last = read_probe_word(hw->gemv, base + PROBE_S2MM_KEEP_LAST);
        uint32_t block_lane = read_probe_word(hw->gemv, base + PROBE_BLOCK_LANE);
        printf("NARROWDBG,%s,%s,%s"
               ...
               case_name, mode_name, rec_names[rec],
               read_probe_word(hw->gemv, base + PROBE_INPUT_INDEX),
               (int32_t)read_probe_word(hw->gemv, base + PROBE_INPUT_I16),
               (int32_t)read_probe_word(hw->gemv, base + PROBE_WEIGHT_I8),
               (int32_t)read_probe_word(hw->gemv, base + PROBE_PRODUCT),
               (int32_t)read_probe_word(hw->gemv, base + PROBE_BLOCK_BEFORE),
               (int32_t)read_probe_word(hw->gemv, base + PROBE_BLOCK_AFTER),
               (int32_t)read_probe_word(hw->gemv, base + PROBE_SCALE_Q),
               (int32_t)read_probe_word(hw->gemv, base + PROBE_SCALED),
               (int32_t)read_probe_word(hw->gemv, base + PROBE_ROW_BEFORE),
               (int32_t)read_probe_word(hw->gemv, base + PROBE_ROW_AFTER),
               (int32_t)read_probe_word(hw->gemv, base + PROBE_OUTPUT_WORD),
               (int32_t)read_probe_word(hw->gemv, base + PROBE_S2MM_TDATA),
               keep_last & 0xfu,
               (keep_last >> 4) & 0x1u,
               (block_lane >> 16) & 0xffffu,
               block_lane & 0xffffu,
               read_probe_word(hw->gemv, base + PROBE_BEAT_IDX),
               read_probe_word(hw->gemv, base + PROBE_OUTPUT_BEAT_IDX));
    }
}
...
static int run_case(struct hw *hw, const struct test_case *tc)
{
    ...
    if (failed) {
        ...
        printf("\n");
        dump_narrow_debug(hw, tc->name, "mode0");
    }
    return failed;
}
...
static int run_case_mode1_blocks(struct hw *hw, const struct test_case *tc)
{
    ...
    if (failed) {
        ...
        printf("\n");
        dump_narrow_debug(hw, tc->name, "mode1");
    }
    return failed;
}
