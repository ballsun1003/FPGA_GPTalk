        ...
        if (reset_p) begin
            ...
            case (state)
                ...
                ST_SCALE_LOAD: begin
                    ...
                    scale_q_reg <= mode_reg ? mode1_identity_scale_q : scale_q[emit_lane];
                    if (emit_lane == 16'd0) begin
                        debug_scale0 <= mode_reg ? mode1_identity_scale_q : scale_q[emit_lane];
                        debug_block0 <= block_acc[emit_lane];
                    end else if (emit_lane == 16'd1) begin
                        debug_scale1 <= mode_reg ? mode1_identity_scale_q : scale_q[emit_lane];
                        debug_block1 <= block_acc[emit_lane];
                    end else if (emit_lane == 16'd2) begin
                        debug_scale2 <= mode_reg ? mode1_identity_scale_q : scale_q[emit_lane];
                ...
                ST_EMIT_ROW: begin
                    if (m_axis_tvalid) begin
                        ...
                        m_axis_tvalid <= 1'b1;
                        m_axis_tlast <= last_group && (mode_reg ? last_block : 1'b1) && emit_last_lane_in_group;
