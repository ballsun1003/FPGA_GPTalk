...
module gemv_q8_0_stream_core #(
    ...
    reg signed [31:0] scale_q_sel;
    reg signed [31:0] block_acc_sel;
    reg signed [ROW_ACC_WIDTH-1:0] row_acc_sel;
    reg signed [31:0] row_out_sel;
    ...
    always @* begin
        case (emit_lane)
            16'd0: begin
                scale_q_sel = scale_q[0];
                block_acc_sel = block_acc[0];
                row_acc_sel = row_acc[0];
                row_out_sel = row_out[0];
            end
            16'd1: begin
                scale_q_sel = scale_q[1];
                block_acc_sel = block_acc[1];
                row_acc_sel = row_acc[1];
                row_out_sel = row_out[1];
            end
            16'd2: begin
                scale_q_sel = scale_q[2];
                block_acc_sel = block_acc[2];
                row_acc_sel = row_acc[2];
                row_out_sel = row_out[2];
            end
            16'd3: begin
                scale_q_sel = scale_q[3];
                block_acc_sel = block_acc[3];
                row_acc_sel = row_acc[3];
                row_out_sel = row_out[3];
            end
            16'd4: begin
                scale_q_sel = scale_q[4];
                block_acc_sel = block_acc[4];
                row_acc_sel = row_acc[4];
                row_out_sel = row_out[4];
            end
            16'd5: begin
                scale_q_sel = scale_q[5];
                block_acc_sel = block_acc[5];
                row_acc_sel = row_acc[5];
                row_out_sel = row_out[5];
            end
            16'd6: begin
                scale_q_sel = scale_q[6];
                block_acc_sel = block_acc[6];
                row_acc_sel = row_acc[6];
                row_out_sel = row_out[6];
            end
            16'd7: begin
                scale_q_sel = scale_q[7];
                block_acc_sel = block_acc[7];
                row_acc_sel = row_acc[7];
                row_out_sel = row_out[7];
            end
            16'd8: begin
                scale_q_sel = scale_q[8];
                block_acc_sel = block_acc[8];
                row_acc_sel = row_acc[8];
                row_out_sel = row_out[8];
            end
            16'd9: begin
                scale_q_sel = scale_q[9];
                block_acc_sel = block_acc[9];
                row_acc_sel = row_acc[9];
                row_out_sel = row_out[9];
            end
            16'd10: begin
                scale_q_sel = scale_q[10];
                block_acc_sel = block_acc[10];
                row_acc_sel = row_acc[10];
                row_out_sel = row_out[10];
            end
            16'd11: begin
                scale_q_sel = scale_q[11];
                block_acc_sel = block_acc[11];
                row_acc_sel = row_acc[11];
                row_out_sel = row_out[11];
            end
            16'd12: begin
                scale_q_sel = scale_q[12];
                block_acc_sel = block_acc[12];
                row_acc_sel = row_acc[12];
                row_out_sel = row_out[12];
            end
            16'd13: begin
                scale_q_sel = scale_q[13];
                block_acc_sel = block_acc[13];
                row_acc_sel = row_acc[13];
                row_out_sel = row_out[13];
            end
            16'd14: begin
                scale_q_sel = scale_q[14];
                block_acc_sel = block_acc[14];
                row_acc_sel = row_acc[14];
                row_out_sel = row_out[14];
            end
            16'd15: begin
                scale_q_sel = scale_q[15];
                block_acc_sel = block_acc[15];
                row_acc_sel = row_acc[15];
                row_out_sel = row_out[15];
            end
            default: begin
                scale_q_sel = 32'sd0;
                block_acc_sel = 32'sd0;
                row_acc_sel = {ROW_ACC_WIDTH{1'b0}};
                row_out_sel = 32'sd0;
            end
        endcase
    end

    wire signed [31:0] block_acc_emit = block_acc_sel;
    wire signed [31:0] scale_q_emit = scale_q_sel;
    wire signed [ROW_ACC_WIDTH-1:0] row_acc_emit = row_acc_sel;
    wire signed [63:0] scale_product_now =
        mul_i32_i32_to_i64(block_acc_emit, scale_q_emit);
    ...
    wire signed [ROW_ACC_WIDTH-1:0] row_acc_after_scalar =
        row_acc_emit + $signed(scaled_shifted_scalar);
    wire signed [31:0] row_out_after_scalar =
        saturate_to_i32(row_acc_after_scalar);
                ...
                ST_WEIGHT_APPLY: begin
                    ...
                    for (b = 0; b < WEIGHTS_PER_WORD; b = b + 1) begin
                        if (apply_lane_base + b < LANES) begin
                                ...
                                $signed(block_acc[apply_lane_base + b]) +
                                $signed(mul_i16_i8_to_i32(input_sample_reg, get_weight_byte(weight_word_ext, b)));
                        end
                    end
