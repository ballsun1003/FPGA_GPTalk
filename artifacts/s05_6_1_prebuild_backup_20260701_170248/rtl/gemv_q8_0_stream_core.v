...
module gemv_q8_0_stream_core #(
    ...
    wire signed [31:0] block_acc_emit = $signed(block_acc[emit_lane]);
    wire signed [31:0] scale_q_emit = $signed(scale_q[emit_lane]);
    wire signed [ROW_ACC_WIDTH-1:0] row_acc_emit = $signed(row_acc[emit_lane]);
    ...
    wire signed [63:0] row_acc_after_scalar_64 = $signed(row_acc_after_scalar);
    wire signed [31:0] row_out_after_scalar =
        (row_acc_after_scalar_64 > 64'sd2147483647) ? 32'sh7fffffff :
        (row_acc_after_scalar_64 < -64'sd2147483648) ? 32'sh80000000 :
            ...
            case (state)
                ...
                ST_EMIT_ROW: begin
                    if (m_axis_tvalid) begin
                        ...
                        m_axis_tdata <= saturate_to_i32($signed(row_acc[emit_lane]));
                        m_axis_tvalid <= 1'b1;
                        ...
                        if (row_group_base + emit_lane_feature == 0) begin
                            debug_out0 <= saturate_to_i32($signed(row_acc[emit_lane]));
                        end else if (row_group_base + emit_lane_feature == 1) begin
                            debug_out1 <= saturate_to_i32($signed(row_acc[emit_lane]));
                        end else if (row_group_base + emit_lane_feature == 2) begin
                            debug_out2 <= saturate_to_i32($signed(row_acc[emit_lane]));
                        end
