...
- Current source is the `fixed_128_mac` stream core plus the `dma_top_output_register` wrapper delta.
...
- The `dma_top_output_register` delta adds a one-entry AXIS register slice between `stream_core` and the S2MM-facing `M_AXIS` port. It does not change core arithmetic/state logic.
